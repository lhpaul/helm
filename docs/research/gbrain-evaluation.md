...
items.
**Subject reviewed:** `garrytan/gbrain` at commit
[`f49ca56`](https://github.com/garrytan/gbrain/tree/f49ca569232dbc0d8e0783d84606115e3bfe5ab1) — every link below is
pinned to that revision, so the note keeps describing what was read even as
`master` moves. Read 2026-08-18; the repository metadata quoted here (MIT,
TypeScript/Bun, created 2026-04-05, ~28.7k stars, pushed the same day) is as of
that date.
...
[README](https://github.com/garrytan/gbrain/blob/f49ca569232dbc0d8e0783d84606115e3bfe5ab1/README.md), are a
...
[`CLAUDE.md`](https://github.com/garrytan/gbrain/blob/f49ca569232dbc0d8e0783d84606115e3bfe5ab1/CLAUDE.md) and
[`docs/architecture/`](https://github.com/garrytan/gbrain/tree/f49ca569232dbc0d8e0783d84606115e3bfe5ab1/docs/architecture).
...
`packages/orchestrator/src/specialists/fetch-product-context.ts` reaches for
exactly four files, by two distinct routes. `materializeProductContext` writes
the code repo's `README.md` and its winning agent-instructions file into the
spec-writer / plan-writer worktree, **untruncated**, so the agent can `cat` and
`grep` them from disk (ADR-030); its sibling `fetchProductContext` fetches the
same two files for the `## Product Context` prompt section, and that is the path
carrying the 2 000-char cap. Separately, `fetchSpecForPlan` and
`fetchPlanForImplementer` read `specs/{externalId}.md` and
`plans/{externalId}.md` from the knowledge repo by path convention (32 000-char
cap) and pass them as prompt input — they are never written to the worktree.
`strategy.md`, `learnings/`, `decisions/`, and `wiki/` are named in ADR-007 as
inputs to Spec Writer, Plan Writer, and Implementer, and no production code path
fetches any of them. ADR-007 anticipated this exact failure —
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Resolver — "task type X → load document Y first"** ([`THIN_HARNESS_FAT_SKILLS.md`](https://github.com/garrytan/gbrain/blob/f49ca569232dbc0d8e0783d84606115e3bfe5ab1/docs/ethos/THIN_HARNESS_FAT_SKILLS.md), Definition 3) | **Adapt the idea**                          | Closes the §2 gap with a lookup table, not an index. Highest value per unit of work in this whole review.                                                                                                                                                                                                                                                                                                                                                    |
| **Executable eval floors** ([BrainBench](https://github.com/garrytan/gbrain/blob/f49ca569232dbc0d8e0783d84606115e3bfe5ab1/docs/eval/BRAINBENCH.md))                                                                         | **Adapt the idea**                          | The transferable mechanic is governance, not the metrics: expectations _pre-registered before the first run_, floors asserted as a test against the committed baseline, and an explicit "a threshold violation can no longer be banked by blessing a new baseline". Helm's external-review contract (ADR-036) is exactly the kind of thing that decays without this.                                                                                         |
...
| **Source attribution / citation discipline** ([`source-attribution.md`](https://github.com/garrytan/gbrain/blob/f49ca569232dbc0d8e0783d84606115e3bfe5ab1/docs/guides/source-attribution.md))                                | **Adapt the idea**                          | Every claim carries who/where/when, with a source-priority order and an explicit rule to record conflicts rather than silently resolve them. Helm's `learnings/` entries have no provenance contract; a one-line "captured from item X, PR Y, finding Z" header is the 90% version.                                                                                                                                                                          |
...
| **Contradiction probe** ([`contradictions.md`](https://github.com/garrytan/gbrain/blob/f49ca569232dbc0d8e0783d84606115e3bfe5ab1/docs/contradictions.md))                                                                    | **Adapt the idea (low priority)**           | Two principles carry over to the review adjudicator (ADR-037/041): the probe **never mutates** — it emits paste-ready commands and lets the operator decide — and findings carry a severity rubric plus a confidence interval, with an explicit "n < 30 is too small to act on". The LLM-judge machinery itself is not worth rebuilding.                                                                                                                     |
| **System-of-record contract** ([`system-of-record.md`](https://github.com/garrytan/gbrain/blob/f49ca569232dbc0d8e0783d84606115e3bfe5ab1/docs/architecture/system-of-record.md))                                             | **Already held — borrow the CI gate later** | gbrain's rule ("the repo is the system of record, the DB is a derived cache, we rebuild rather than back up") _is_ ADR-007 rule 5. The part Helm does not have is the enforcement: a CI gate that fails any PR adding a direct write to a derived table outside the reconciler. Only relevant if Helm ever builds the index ADR-007 leaves open — file it against that day, not today.                                                                       |
...
| **Adopting gbrain as a dependency or fork**                                                                                                                                                                                 | **Discard**                                 | The MIT licence permits reusing gbrain's code, subject to retaining its copyright and permission notices — and nothing below proposes copying code, only adapting principles — so the licence is not the constraint. The ops surface is: a Postgres/PGLite brain, an embedding provider, an enrichment pipeline, 100+ MCP operations, and a cron fleet, against a v0 that explicitly ships with no database. Every row above is reachable by cherry-picking. |
