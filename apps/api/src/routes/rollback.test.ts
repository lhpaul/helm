...
describe('POST /api/items/:externalId/rollback', () => {
  it('rolls an in-development item back to plan-ready and records the history entry', async () => {
    ...
    // Writeback (ADR-033): manual:rollback is not tracker-originated, so the
    // rolled-back stage (plan-ready) is pushed back to the tracker.
    expect(mockSetSubStage).toHaveBeenCalledWith('HLM-1', 'plan-ready');
